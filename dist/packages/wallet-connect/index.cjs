...
var uncurryThis$i = NATIVE_BIND$3 && bind$5.bind(call$h, call$h);
...
var isSymbol$2 = USE_SYMBOL_AS_UID ? function (it) {
  ...
  return isCallable$j($Symbol) && isPrototypeOf$2($Symbol.prototype, $Object$2(it));
};
...
var ordinaryToPrimitive$1 = function (input, pref) {
  var fn, val;
  if (pref === 'string' && isCallable$h(fn = input.toString) && !isObject$5(val = call$f(fn, input))) return val;
  if (isCallable$h(fn = input.valueOf) && !isObject$5(val = call$f(fn, input))) return val;
  if (pref !== 'string' && isCallable$h(fn = input.toString) && !isObject$5(val = call$f(fn, input))) return val;
...
objectGetOwnPropertyDescriptor.f = DESCRIPTORS$5 ? $getOwnPropertyDescriptor : function getOwnPropertyDescriptor(O, P) {
  ...
  if (hasOwn$6(O, P)) return createPropertyDescriptor$2(!call$d(propertyIsEnumerableModule$1.f, O, P), O[P]);
};
  ...
  return [
    ...
    function split(separator, limit) {
      ...
      return splitter
        ...
        : call$8(internalSplit, toString(O), separator, limit);
    },
